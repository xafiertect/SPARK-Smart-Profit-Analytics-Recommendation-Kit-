...
export default function AiChat() {
  ...
  const sendMessage = async (text) => {
    ...
    try {
      ...
      setMessages((p) => [...p, { id: Date.now() + 1, sender: 'ai', text: 'Maaf, ada kendala. Coba lagi ya! 🙏', time: 'Baru saja' }]);
    } finally {
      setIsTyping(false);
    }
  };
  ...
  return (
    <div className="chat-page">
      <div className="chat-page__header">
        ...
        <div className="chat-page__header-info"><h2>SPARK AI</h2><p>● Online</p></div>
      </div>
      <div className="chat-page__messages" ref={messagesRef}>
        {messages.length === 0 && (
          <div className="chat-page__welcome">
            ...
            <p>Tanyakan apa saja tentang bisnis kamu.</p>
          </div>
        )}
        {messages.map((m) => <ChatBubble key={m.id} message={m.text} sender={m.sender} time={m.time} />)}
        {isTyping && <TypingIndicator />}
      </div>
      {messages.length === 0 && (
        <div className="chat-page__suggestions">
          {SUGGESTIONS.map((q) => <button key={q} className="chat-page__suggestion" onClick={() => sendMessage(q)}>{q}</button>)}
        </div>
      )}
      <form className="chat-page__input-area" onSubmit={(e) => { e.preventDefault(); sendMessage(input); }}>
        <input className="chat-page__input" placeholder="Tanya sesuatu..." value={input} onChange={(e) => setInput(e.target.value)} id="chat-input" />
        <button className="chat-page__send" type="submit" disabled={!input.trim() || isTyping} id="btn-send-chat"><Send size={20} /></button>
      </form>
    </div>
  );
}
