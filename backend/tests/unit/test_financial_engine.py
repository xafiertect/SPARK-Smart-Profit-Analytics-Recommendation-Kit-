...
from tests.fixtures.mock_business_context import make_transactions, make_expenses

def test_profit_calculation():
    transactions = make_transactions(sales=500_000)
    expenses = make_expenses(purchases=300_000)
    result = _calculate_summary(transactions, expenses)
...
def test_only_purchases_loss():
    expenses = make_expenses(purchases=300_000)
    result = _calculate_summary([], expenses)
    assert result["income"] == 0
    assert result["expense"] == 300_000
